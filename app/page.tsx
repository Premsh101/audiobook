...
function Icon({ name, size = 18 }: { name: string; size?: number }) {
  ...
  if (name === "heart") return <svg viewBox="0 0 24 24" {...common}><path d="M20.8 8.9c0 5.3-8.8 10-8.8 10s-8.8-4.7-8.8-10A4.8 4.8 0 0 1 8 4.2c1.6 0 3.1.8 4 2.1a4.8 4.8 0 0 1 8.8 2.6Z"/></svg>;
  if (name === "clock") return <svg viewBox="0 0 24 24" {...common}><circle cx="12" cy="12" r="8.5"/><path d="M12 7.5v5l3.2 1.9"/></svg>;
  if (name === "volume") return <svg viewBox="0 0 24 24" {...common}><path d="M5 10h3l4-3v10l-4-3H5z"/><path d="M15.5 9.2a4 4 0 0 1 0 5.6M18 7a7 7 0 0 1 0 10"/></svg>;
  if (name === "x") return <svg viewBox="0 0 24 24" {...common}><path d="m7 7 10 10M17 7 7 17"/></svg>;
  if (name === "chevron") return <svg viewBox="0 0 24 24" {...common}><path d="m9 6 6 6-6 6"/></svg>;
  if (name === "back") return <svg viewBox="0 0 24 24" {...common}><path d="m15 6-6 6 6 6"/></svg>;
  return null;
}

function Cover({ book, large = false }: { book: Book; large?: boolean }) {
  return (
    <div className={large ? "cover cover-large" : "cover"} style={{ "--cover": book.cover, "--ink": book.coverInk } as React.CSSProperties}>
      ...
      <div className="player">
        ...
        <div className="player-main">
          <div className="player-title-line">
            <div>
              <strong>{book.title}</strong>
              <span>{book.author} · free preview from LibriVox</span>
            </div>
          ...
          <div className="progress-wrap">
            <input type="range" min="0" max={duration || 1} value={current} onChange={(e) => { const value = Number(e.target.value); setCurrent(value); if (audioRef.current) audioRef.current.currentTime = value; }} aria-label="Audio progress"/>
  ...
  return (
    <div className="site">
      <header className="topbar">
        <div className="nav-wrap">
          ...
          <nav>
            ...
            <a href="#classics">Library</a>
            <a href="#voice">Favourite Voice</a>
            <a href="#plans">Plans</a>
          </nav>
          <div className="nav-actions">
            <button className="search-trigger"><Icon name="search" size={17}/><span>Search</span></button>
            <Link className="nav-ghost" href="/voice">Sign in</Link>
            <Link className="nav-cta" href="/voice">Try Favourite Voice <Icon name="arrow" size={14}/></Link>
          </div>
        </div>
      </header>
      ...
      <main>
        <section className="hero container" id="discover">
          <div className="hero-copy">
            ...
            <div className="hero-actions">
              <a className="button-dark" href="#classics">Explore free books <Icon name="arrow" size={16}/></a>
              <Link className="button-light" href="/voice"><Icon name="mic" size={17}/> Hear a favourite voice</Link>
            </div>
            <div className="trust-row">
              <span><b>10</b> free classics</span>
              <span><b>5 min</b> voice preview</span>
              <span><b>Global</b> by design</span>
            </div>
          </div>
          <div className="hero-stage">
            ...
            <div className="hero-player-chip">
              <div className="chip-cover"><Cover book={books[1]}/></div>
              <div><span>Now previewing</span><strong>Alice in Wonderland</strong></div>
              <button onClick={() => setSelected(books[1])}><Icon name="play" size={14}/></button>
            </div>
          </div>
        </section>

        <section className="container intro-strip">
          <div><span className="section-eyebrow">THE DIFFERENCE</span><strong>One library. One personal layer.</strong></div>
          <p>Familiar audiobook discovery, with a new reason to press play: hearing stories in a voice that already means something.</p>
        </section>

        <section className="container library-shell" id="classics">
          <div className="library-toolbar">
            <div>
              <span className="section-eyebrow">FREE LIBRARY</span>
              <h2>Start with something everyone knows.</h2>
            </div>
            <div className="search-box"><Icon name="search" size={16}/><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search books or authors…" aria-label="Search books"/></div>
          </div>
          ...
          {filtered.length > 0
            ? <div className="book-grid">{filtered.map((book) => <BookCard key={book.id} book={book} onSelect={setSelected}/>)}</div>
            : <div className="empty">No classics match that search yet.</div>}
        </section>

        <section className="container">
          <Shelf title="For slow mornings" eyebrow="EDITOR'S SHELF" items={books.slice(0, 5)} onSelect={setSelected}/>
          <Shelf title="When you want a little mystery" eyebrow="AFTER DARK" items={[books[2], books[3], books[4], books[9], books[7]]} onSelect={setSelected}/>
        </section>

        <section className="container voice-cta" id="voice">
          <div className="voice-copy">
            <span className="section-eyebrow">FAVOURITE VOICE</span>
            <h2>Imagine hearing a familiar voice tell your favourite story.</h2>
            <p>Upload a clean recording from someone you have permission to clone. Listen to a five-minute sample free. Buy minutes only when you want more.</p>
            <div className="voice-buttons"><Link className="button-cream" href="/voice"><Icon name="mic" size={17}/> Create a Favourite Voice</Link><span>5 minutes free · no card required</span></div>
          </div>
          <div className="voice-demo">
            <div className="demo-top"><div className="demo-avatar">M</div><div><span>Favourite Voice</span><strong>Mom's voice</strong></div><span className="demo-private">PRIVATE</span></div>
            <div className="waveform">{[14,32,20,45,28,62,38,19,52,31,66,40,21,47,25,58,35,22,50,29].map((h, i) => <i key={i} style={{ height: h + "px" }}/>)}</div>
            <div className="demo-status"><span>00:00 → 05:00</span><strong>Free preview</strong></div>
            <div className="demo-foot"><span>Natural pacing</span><span>Expressive delivery</span><span>Consent-first</span></div>
          </div>
        </section>
        ...
        <section className="container plan-section" id="plans">
          ...
          <div className="plan-cards">
            <div className="plan-card"><span>LIBRARY</span><strong>From $2.99</strong><p>Monthly access to the free-to-you catalog and new titles as the library grows.</p><button>View library plans <Icon name="arrow" size={15}/></button></div>
            <div className="plan-card featured"><span>FAVOURITE VOICE</span><strong>5 min free</strong><p>Then buy listening-minute credits when you want a longer personalised story.</p><Link href="/voice">Try the free preview <Icon name="arrow" size={15}/></Link></div>
          </div>
        </section>
      ...
      <footer className="container footer">
        <div className="footer-main"><Link href="/" className="brand">hush<span>.</span></Link><span>Stories, closer.</span></div>
        <div className="footer-links"><a href="#classics">Library</a><a href="#voice">Favourite Voice</a><a href="#plans">Plans</a><a href={books[0].sourceUrl} target="_blank" rel="noreferrer">Open source</a></div>
      </footer>
